...
//   session_start();
  ...
  $selPass="select * from tbl_admin where admin_id='".$_SESSION['aid']."'";
  ...
  if(isset($_POST['pass_submit']))
  {
    ...
    if($row['admin_password'] == $old_p)
    {
        if($new_p == $confirm_p)
        {
            // $update="update tbl_admin set admin_password='".$new_p."' where admin_id=".$_SESSION['uid'];
            $update="UPDATE `tbl_admin` SET `admin_password` = '".$new_p."' WHERE `tbl_admin`.`admin_id` = '".$_SESSION['aid']."'";
            $con->query($update);
            ?>
            <script>
