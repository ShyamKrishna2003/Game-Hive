...
<?php
   ...
   $selQry =  "select * from tbl_community where community_id='".$_GET["id"]."'";
...
<div class="chat-nav">
    <div class="nav-start">
        <div class="recipient-block">
            <div class="avatar-container">
                <img class="user-avatar" src="../Assets/File/Community/<?php echo $row["community_photo"];?>" alt="">
            </div>
            <div class="username">
    ...
    <div class="nav-end">
        <a href="CommunityHomepage.php" class="chat-nav-item is-icon is-hidden-mobile">
...
<div id="chat-sidebar" class="users-sidebar">   
    ...
    <div class="conversations-list has-slimscroll-xs">
        <!-- User -->
        <?php
                $selQry1 =  "select * from tbl_community cm inner join tbl_user u on u.user_id=cm.user_id where community_id='".$_GET["id"]."'";
                $result1=$con->query($selQry1);
            while ($row1=$result1->fetch_assoc()) {
        ?>
        <div class="user-item">
            <div class="avatar-container">
                <img class="user-avatar" style="width: 32px; height:32px "  src="../Assets/File/User/<?php echo $row1["user_photo"];?>" alt="<?php echo $row1["user_photo"];?>">
            </div>
        </div>
        <?php
            }
        ?>
        <?php
                $selQry1 =  "select * from tbl_joinlist cm inner join tbl_user u on u.user_id=cm.user_id where community_id='".$_GET["id"]."'";
                $result1=$con->query($selQry1);
            while ($row1=$result1->fetch_assoc()) {
        ?>
        <div class="user-item">
        ...
        <script>
                                    ...
                                    $(document).ready(function() {
                                        setInterval(function() {
                                            ...
                                            $("#conversation").load('AjaxLoad.php?cid='+cid)
                                        }, 50);
                                    });
